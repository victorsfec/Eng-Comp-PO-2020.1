...
class binaryTree:
    ...
    def balanceamento(self, pai):
        ...
        while len(no) < self.baixo - 1:  # balanceamento tree from down to up
            ...
            no_esquerdo = [] if filhoposicao == 0 else prt.obterfilho(
                filhoposicao - 1)
            no_direito = [] if filhoposicao == len(
                prt) else prt.obterfilho(filhoposicao + 1)
    ...
    def __str__(self):
        ...
        cauda = '-' * 30 + 'the end' + '-' * 30 + '\n'
        lista = [[cabeca], [
            f'No num: {self.NoNum},  chave num: {self.chaveNum}']]
        cur = []
        noNum = 0
        noTotal = 1
        que = [self.raiz]
